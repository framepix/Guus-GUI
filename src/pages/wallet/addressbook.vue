...
<q-page class="address-book">
    ...
    <template v-if="address_book_combined.length">
        <q-list link no-border :dark="theme=='dark'" class="guus-list">
            <q-item class="guus-list-item" v-for="(entry, index) in address_book_combined" @click.native="details(entry)" :key="`${entry.address}-${entry.name}`">
                <q-item-main>
