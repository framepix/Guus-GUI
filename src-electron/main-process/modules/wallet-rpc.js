...
export class WalletRPC {
                ...
                portscanner.checkPortStatus(this.port, this.hostname).catch(e => "closed").then(status => {
                    if (status === "closed") {
                        if (process.platform === "win32") {
                            this.walletRPCProcess = child_process.spawn(path.join(__ryo_bin, "guus-wallet-rpc.exe"), args)
                        } else {
                            this.walletRPCProcess = child_process.spawn(path.join(__ryo_bin, "guus-wallet-rpc"), args, {
                                detached: true
                            })
                        }
    ...
    handle (data) {
        ...
        case "restore_view_wallet":
            // TODO: Decide if we want this for guus
        ...
        case "register_frame_pix":
            this.registerSnode(params.password, params.string)
            break

        case "unlock_stake":
            this.unlockStake(params.password, params.frame_pix_key, params.confirmed || false)
            break
    ...
    stake (password, amount, frame_pix_key, destination) {
    ...
    registerSnode (password, register_frame_pix_str) {
        crypto.pbkdf2(password, this.auth[2], 1000, 64, "sha512", (err, password_hash) => {
            if (err) {
                this.sendGateway("set_snode_status", {
                    registration: {
            ...
            this.sendRPC("register_frame_pix", {
                register_frame_pix_str
            }).then((data) => {
                if (data.hasOwnProperty("error")) {
    ...
    unlockStake (password, frame_pix_key, confirmed = false) {
    ...
    listWallets (legacy = false) {
        ...
        if (legacy) {
            ...
            if (os.platform() == "win32") {
                legacy_paths = ["C:\\ProgramData\\Guus"]
            } else {
                legacy_paths = [path.join(os.homedir(), "Guus")]
            }
