...
export class Daemon {
    ...
    checkVersion () {
        return new Promise((resolve, reject) => {
            if (process.platform === "win32") {
                let guusd_path = path.join(__ryo_bin, "guusd.exe")
                let guusd_version_cmd = `"${guusd_path}" --version`
                if (!fs.existsSync(guusd_path)) { resolve(false) }
                child_process.exec(guusd_version_cmd, (error, stdout, stderr) => {
                    if (error) { resolve(false) }
                    resolve(stdout)
                })
            } else {
                let guusd_path = path.join(__ryo_bin, "guusd")
                let guusd_version_cmd = `"${guusd_path}" --version`
                if (!fs.existsSync(guusd_path)) { resolve(false) }
                child_process.exec(guusd_version_cmd, { detached: true }, (error, stdout, stderr) => {
                    if (error) { resolve(false) }
                    resolve(stdout)
                })
            }
    ...
    start (options) {
        ...
        return new Promise((resolve, reject) => {
            ...
            args.push("--log-file", path.join(dirs[net_type], "logs", "guusd.log"))
            ...
            portscanner.checkPortStatus(this.port, this.hostname).catch(e => "closed").then(status => {
                if (status === "closed") {
                    if (process.platform === "win32") {
                        this.daemonProcess = child_process.spawn(path.join(__ryo_bin, "guusd.exe"), args)
                    } else {
                        this.daemonProcess = child_process.spawn(path.join(__ryo_bin, "guusd"), args, {
                            detached: true
                        })
                    }
    ...
    updateFramePixs () {
        ...
        this.getRPC("frame_pixs").then(data => {
            ...
            const states = data.result.frame_pix_states

            // Only store the data we need
            const frame_pixs = states.map(s => ({
                frame_pix_pubkey: s.frame_pix_pubkey,
                contributors: s.contributors
            }))
            this.sendGateway("set_daemon_data", { frame_pixs })
        })
    }
